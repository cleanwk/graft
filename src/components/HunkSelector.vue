...
async function load() { busy.value = true; error.value = ""; try { [staged.value, unstaged.value] = await Promise.all([api.hunks(props.repositoryPath, props.file, true), api.hunks(props.repositoryPath, props.file, false)]); } catch (caught) { error.value = String(caught); } finally { busy.value = false; } }
async function apply(hunk: DiffHunk, isStaged: boolean) { busy.value = true; try { const snapshot = await api.applyHunk(props.repositoryPath, props.file, isStaged, hunk.index); emit("changed", snapshot); await load(); } catch (caught) { error.value = String(caught); busy.value = false; } }
        ...
        <section v-for="group in allHunks" :key="group.label">
          ...
          <div class="hunk-list">
            <article v-for="hunk in group.hunks" :key="hunk.index" class="hunk-card">
              ...
              <pre><code><span v-for="(line, index) in hunk.patch.split('\n').filter(line => line.startsWith('@@') || (!line.startsWith('diff ') && !line.startsWith('index ') && !line.startsWith('---') && !line.startsWith('+++')))" :key="index" :class="lineClass(line)">{{ line }}
</span></code></pre>
            </article>
