...
async function load() { busy.value = true; try { conflict.value = await api.conflict(props.repositoryPath, props.file); result.value = conflict.value.working; } catch (caught) { error.value = String(caught); } finally { busy.value = false; } }
function use(value: string) { result.value = value; nextTick(() => document.querySelector<HTMLTextAreaElement>(".merge-result")?.focus()); }
async function save() { busy.value = true; try { const response = await api.resolveConflict(props.repositoryPath, props.file, result.value); emit("complete", response.summary); } catch (caught) { error.value = String(caught); } finally { busy.value = false; } }
</script>
  ...
  <div class="resolver-backdrop">
    <section class="resolver" role="dialog" aria-modal="true" aria-labelledby="resolver-title">
      ...
      <div v-if="conflict" class="merge-grid">
        ...
        <section class="result-pane"><div class="merge-label"><strong>Result</strong><span>Edit the final file, then mark it resolved.</span></div><textarea v-model="result" class="merge-result" spellcheck="false" aria-label="Merge result" /></section>
      </div>
