...
export interface GraphRow { lane: number; before: number; nextLanes: number[]; parentLanes: number[]; introducedParentLanes: number[] }
...
export function allocateGraphRows(commits: CommitRow[], maxLanes = 8): GraphRow[] {
  ...
  return commits.map((commit) => {
    ...
    const introducedParents = new Set<string>();
    commit.parents.slice(1).forEach((parent, offset) => {
      if (!lanes.includes(parent)) { lanes.splice(lane + 1 + offset, 0, parent); introducedParents.add(parent); }
    });
    ...
    const introducedParentLanes = [...introducedParents].map((parent) => lanes.indexOf(parent)).filter((value) => value >= 0 && value < maxLanes);
    return {
      lane,
      before,
      nextLanes: lanes.slice(0, maxLanes).map((_, index) => index).filter((index) => !introducedParentLanes.includes(index)),
      parentLanes,
      introducedParentLanes,
    };
  });
}
